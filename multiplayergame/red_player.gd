...
func get_input():
	...
	var secondAT = Input.is_action_pressed('secondAT')
	var thirdAT = Input.is_action_pressed('thirdAT')
	velocity.x=0
	#-----------------------------------------------------------movement
	if right:
	...
	if firstAT and not cooldown_active and is_on_floor(): #j
		...
		await get_tree().create_timer(0.5).timeout
		change_state(IDLE)
	if secondAT and not cooldown_active and is_on_floor():#k
		hide_all_sprites(self)
		$hit2.visible=true
		$AnimationPlayer.play('hit2')
		
		if righthit ==true:
			$hit2colarR/hit2col.disabled=false
		if lefthit ==true:
			$hit2colarL/hit2col.disabled=false
		
		$sword2.play()#sound effect
		cooldown_active=true
		$Timer.start()
		await get_tree().create_timer(0.5).timeout
		change_state(IDLE)
	if thirdAT and not cooldown_active and is_on_floor(): #L
		hide_all_sprites(self)
		$hit3.visible=true
		$AnimationPlayer.play('hit3')
		
		if righthit ==true:
			$hit3colarR/hit3col.disabled=false
		if lefthit ==true:
			$hit3colarL/hit3col.disabled=false
		
		$sword3.play()#sound effect
		cooldown_active=true
		$Timer.start()
...
func hide_all_sprites(node):
	...
	$hit2colarL/hit2col.disabled=true
	$hit2colarR/hit2col.disabled=true
	$hit3colarL/hit3col.disabled=true
	$hit3colarR/hit3col.disabled=true
